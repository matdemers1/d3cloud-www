import { CONTACT_EMAIL } from '../content/projects';
import { WORKSHOP, type WorkshopItem } from '../content/ecosystem';
...
export function WorkshopPage({ item }: { item: WorkshopItem }) {
  ...
  return (
    <>
      <section aria-labelledby="workshop-title" className="relative overflow-hidden">
        <div className={`${WRAP} flex flex-col gap-7 pt-16 pb-14 lg:pt-24`}>
          ...
          <div className="stagger-3 flex animate-rise flex-wrap items-center gap-3 pt-2">
            <PrimaryButton href={item.cta.href}>{item.cta.label} ↗</PrimaryButton>
            <SecondaryButton href="/#log">Everything on the bench</SecondaryButton>
          </div>
        </div>
      ...
      <Band label="Where it stands" title={item.stage} sunken>
        <div className="flex max-w-3xl flex-col gap-5">
          ...
          <p className="text-14 text-fg-muted">
            {built.length} of {item.features.length} features below are built
            {planned.length ? `; ${planned.length} are planned` : ''}.
          </p>
        </div>
      </Band>
      ...
      <Band label="What it does" title="Built, and planned." sunken>
        <ul className="flex max-w-3xl flex-col divide-y divide-border">
          {item.features.map((feature) => (
            <li key={feature.text} className="flex items-start justify-between gap-6 py-4 first:pt-0">
              <span className={`text-16 ${feature.built ? 'text-fg' : 'text-fg-muted'}`}>
                {feature.text}
              </span>
